...
This is a Jekyll site at the most basic functional level. Here's what is happening:
  ...
  * Jekyll looks for files with front matter tags(the two sets of dashed lines `---` like those in `index.md`) and processes the files (populating site variables, rendering any [Liquid](https://shopify.github.io/liquid/), and converting Markdown to HTML).
...
You can read more about how Jekyll processes the files in [order of Interpretation]({% link docs/Jekyll/Jekyll Tutorial/orderofinterpretation.md %}).
...
Jekyll provides some [filters]({% link docs/Jekyll/Jekyll Home/Jekyll_docs/filters.md %}) to prepend a site URL before path. For example, you could preface your stylesheet like this:
...
You can even set [default front matter tags]({% link docs/Jekyll/Jekyll Home/Jekyll_docs/front-matter-defaults.md %}) for pages, posts, or [collections]({% link docs/Jekyll/Jekyll Home/collections.md %}) in your `_config.yml` file so that you don't have to specify the layout in the front matter variables. Anyways, setting defaults is beyond the scope of this tutorial, let's get back to work.
...
You can also package your layouts, includes and assets into a Ruby `gem` and [make it a Jekyll theme]({% link docs/Jekyll/Jekyll Home/themes.md %}).
